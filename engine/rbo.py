...
1. **Predicate Pushdown** - Move a SelectNode (WHERE filter) as *close to
   ...
   **AND clauses** (``is_or_block=False``): Each split clause is pushed
   independently.  A clause that references only one table's columns is pushed
   to that table's subtree.

   **OR blocks** (``is_or_block=True``): An OR-compound predicate can only be
   pushed down if ALL table references in the expression belong to the SAME
   side of the join.  If the OR spans multiple tables it is mathematically
   impossible to evaluate independently on either side  — the SelectNode must
   stay above the JoinNode.  A descriptive rule entry is recorded so the UI
   can explain why the predicate was not pushed.

   **AggregateNode barrier**: A predicate referencing an aggregate function
   (``COUNT``, ``SUM``, ``AVG``, ``MIN``, ``MAX``) must stay above the
   AggregateNode that produced those values.

   **SubqueryNode barrier**: Predicates are **not** pushed into subquery
   sources from outside — the inner plan is already independently optimised.

   **LEFT/RIGHT JoinNode barrier for right-side predicates**: For a LEFT JOIN,
   a filter on the *right* table cannot be pushed below the join without
   changing semantics (NULL-padding rows would be eliminated).  Such predicates
...
2. **Projection Pushdown** - Determine the exact set of columns required by
   ...
   unused columns are dropped as early as possible.
